...
//                DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendDataImmediately( data );
    ...
    void startPrinting() {
        stack.push(new Pair("GEE9521594514963SG", true));
        stack.push(new Pair("GEE9521594514964SG", false));
        printStart = true;
        checkForPending();
    }
    ...
    private void checkForPending() {
        ...
        threadPool.addTask(new Runnable() {
            ...
            public void run() {
                ...
                if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand() == PrinterCommand.TSC) {
                    ThreadFactoryBuilder threadFactoryBuilder = new ThreadFactoryBuilder("MainActivity_sendLabel_Timer");
                    ScheduledExecutorService scheduledExecutorService = new ScheduledThreadPoolExecutor(1, threadFactoryBuilder);
                    scheduledExecutorService.schedule(threadFactoryBuilder.newThread(new Runnable() {
                        @Override
                        public void run() {
     ...
     * 发送标签
     */
    void sendLabel(final String barCode, Boolean enableWidth) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(MainActivity.this, "Printing: " + barCode, Toast.LENGTH_SHORT).show();
            }
        });
...
//        tsc.addQRCode( 10, 330, LabelCommand.EEC.LEVEL_L, 5, LabelCommand.ROTATION.ROTATION_0, "Printer" );
        /* 绘制一维条码 */
        if (enableWidth) {
            tsc.add1DBarcode(50, 50, LabelCommand.BARCODETYPE.CODE128, 150, LabelCommand.READABEL.EANBEL, LabelCommand.ROTATION.ROTATION_0, 0, 300, barCode);
        } else {
            tsc.add1DBarcode(50, 50, LabelCommand.BARCODETYPE.CODE128, 150, LabelCommand.READABEL.EANBEL, LabelCommand.ROTATION.ROTATION_0, barCode);
        }
...
//        tsc.addQRCode( 250, 80, LabelCommand.EEC.LEVEL_L, 5, LabelCommand.ROTATION.ROTATION_0, " Printer" );
        /* 绘制一维条码 */
        tsc.add1DBarcode(20, 20, LabelCommand.BARCODETYPE.CODE128, 100, LabelCommand.READABEL.EANBEL, LabelCommand.ROTATION.ROTATION_0, "SMARNET");
        /* 打印标签 */
        tsc.addPrint(1, 1);
        /* 打印标签后 蜂鸣器响 */
